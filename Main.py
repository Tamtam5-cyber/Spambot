...
async def spam_contacts(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    while config["is_spamming"]:
        ...
        config = get_user_config(chat_id, conn)
        if config["replay"] and config["is_spamming"]:
    ...
    save_user_config(chat_id, config, conn)
    await update.message.reply_text("Spam đã dừng.")

# Hàm spam từ file số điện thoại
async def spam_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_id = update.effective_chat.id
    if not is_user_logged_in(chat_id, conn):
        await update.message.reply_text("Vui lòng đăng nhập trước! Sử dụng /dangnhap phone_number hoặc /themsession")
        return
    ...
    config["is_spamming"] = True
    save_user_config(chat_id, config, conn)
    await update.message.reply_text("Vui lòng gửi file CSV chứa số điện thoại (cột 'phone').")
    
    # Chờ người dùng gửi file
    context.user_data["awaiting_file"] = True

# Hàm xử lý file người dùng gửi
async def handle_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    file = await update.message.document.get_file()
    file_path = f"sessions/{chat_id}/phone_numbers.csv"
    os.makedirs(f"sessions/{chat_id}", exist_ok=True)
    await file.download_to_drive(file_path)
    
    # Đọc số điện thoại từ file
    phone_numbers = read_phone_numbers_from_file(file_path)
    if not phone_numbers:
        await update.message.reply_text("Không tìm thấy số điện thoại trong file. Đảm bảo file có cột 'phone'.")
        return
    
    await update.message.reply_text("Đã nhận file. Bắt đầu spam...")
...
async def start_spam(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    config["is_spamming"] = True
    save_user_config(chat_id, config, conn)
    await update.message.reply_text("Starting to spam >>>>>")
    ...
    while config["is_spamming"]:
        for group_id in joined_groups:
            ...
            if not config["is_spamming"]:
                break
            await context.bot.send_message(chat_id=group_id, text="Đây là tin nhắn spam!")
            await asyncio.sleep(config["delay"])
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "stop_spam":
        config["is_spamming"] = False
        save_user_config(chat_id, config, conn)
        await query.message.reply_text("Spam đã dừng.")
    
    elif query.data == "set_delay":
        await query.message.reply_text("Nhập thời gian delay mới (giây):")
        # Thêm logic để nhận input từ người dùng
    
    elif query.data == "toggle_replay":
        config["replay"] = not config["replay"]
        save_user_config(chat_id, config, conn)
        await query.message.reply_text(f"Replay: {'YES' if config['replay'] else 'NO'}")
        await show_config(query, context)
    
    elif query.data == "set_delay_replay":
        await query.message.reply_text("Nhập thời gian delay replay mới (giây):")
        # Thêm logic để nhận input từ người dùng
...
def main():
    ...
    application.add_handler(CommandHandler("dangnhap", dangnhap))
    application.add_handler(CommandHandler("xacthuc", xacthuc))
    application.add_handler(CommandHandler("themsession", themsession))
    ...
    application.add_handler(CommandHandler("startspam", start_spam))
    application.add_handler(CommandHandler("spamcontacts", spam_contacts))
    application.add_handler(CommandHandler("spamfile", spam_file))
    application.add_handler(MessageHandler(filters.Document.ALL, handle_file))
    application.add_handler(MessageHandler(filters.ALL, handle_new_message))
